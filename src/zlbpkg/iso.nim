...
proc layoutIsoTree(p: ProjectPaths, arch: string, sfsPath, brandingDir: string): string =
  ...
  removeDir(isoTree)
  createDir(isoTree / "zenit" / arch)
  createDir(isoTree / "boot" / "grub")
  copyFileWithPermissions(sfsPath, isoTree / "zenit" / arch / "airootfs.sfs")
...
proc writeGrubCfg(isoTree, distroName, arch: string) =
  ...
  ## `installer=1` na linii poleceń jądra jest tym, co (w zenit-main's
  ## overlays/system/usr/local/bin/zenit-session-select) odróżnia sesję
  ## live-only od pełnoekranowego Zenit Installer -- podobnie jak
...
menuentry "Try/Live {distroName} ({arch})" {{
  linux /boot/vmlinuz-linux boot=zenit arch={arch} quiet
  initrd /boot/initramfs-linux.img
}}

menuentry "Install {distroName} ({arch})" {{
  linux /boot/vmlinuz-linux boot=zenit arch={arch} installer=1 quiet
  initrd /boot/initramfs-linux.img
}}

menuentry "{distroName} ({arch}) - Safe graphics" {{
  linux /boot/vmlinuz-linux boot=zenit arch={arch} nomodeset
  initrd /boot/initramfs-linux.img
}}
"""
